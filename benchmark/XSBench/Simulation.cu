...
__global__ void xs_lookup_kernel_baseline(Inputs in, SimulationData GSD )
{
	...
	double macro_xs_vector2[5] = {0};
		calculate_macro_xs(
			...
			#endif
    #else
    auto grad = clad::gradient(calculate_macro_xs, "nuclide_grids, macro_xs_vector");
...
// returns lower index
__host__ __device__ long grid_search( long n, double quarry, const double * __restrict__ A)
{
	long lowerLimit = 0;
	long upperLimit = n-1;
