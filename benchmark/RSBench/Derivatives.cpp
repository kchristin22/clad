...
namespace clad
{
    namespace custom_derivatives
    {
        namespace class_functions
        {
            __device__ void constructor_pullback(Input &other, Input *_d_this,
                                                 Input *_d_arg){
                {
					(*_d_arg).kernel_id += _d_this -> kernel_id;
					_d_this->kernel_id = 0;
				}
				{
					(*_d_arg).simulation_method += _d_this->simulation_method;
					_d_this->simulation_method = 0;
				}
				{
					(*_d_arg).particles += _d_this->particles;
					_d_this->particles = 0;
				}
				{
					(*_d_arg).doppler += _d_this->doppler;
					_d_this->doppler = 0;
				}
				{
					(*_d_arg).numL += _d_this->numL;
					_d_this->numL = 0;
				}
				{
					(*_d_arg).avg_n_windows += _d_this->avg_n_windows;
					_d_this->avg_n_windows = 0;
				}
				{
					(*_d_arg).avg_n_poles += _d_this->avg_n_poles;
					_d_this->avg_n_poles = 0;
				}
				{
					_d_this->HM = static_cast<HM_size>(0);
				}
				{
					(*_d_arg).lookups += _d_this->lookups;
					_d_this->lookups = 0;
				}
				{
					(*_d_arg).n_nuclides += _d_this->n_nuclides;
					_d_this->n_nuclides = 0;
				}
				{
					(*_d_arg).nthreads += _d_this->nthreads;
					_d_this->nthreads = 0;
				}
            };
        }
